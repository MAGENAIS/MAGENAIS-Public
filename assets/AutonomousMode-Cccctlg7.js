import{C as e,G as t,K as n,N as r,O as i,R as a,S as o,V as s,_ as c,b as l,q as u,u as d,v as f,x as p,y as m}from"./main-N2xr38iN.js";import{n as h,r as g,t as _}from"./modes-DJUDz4W7.js";import{t as v}from"./Mode-Bm6SiWub.js";import{t as y}from"./VoiceInput-CvaLwtaL.js";function b(e){return e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function x(e){return new Promise(t=>{let n=document.getElementById(`autonomousModelConsentModal`);n&&n.remove();let r=document.createElement(`div`);r.className=`modal-backdrop open`,r.id=`autonomousModelConsentModal`,r.innerHTML=`
    ...
    `,document.body.appendChild(r);let i=e=>{r.remove(),t(e)};r.querySelector(`#autoGapClose`)?.addEventListener(`click`,()=>i(`cancel`)),r.querySelector(`#autoGapCancel`)?.addEventListener(`click`,()=>i(`cancel`)),r.querySelector(`#autoGapSwitchHybrid`)?.addEventListener(`click`,()=>i(`switch-hybrid`)),r.querySelector(`#autoGapOpenProviders`)?.addEventListener(`click`,()=>i(`open-providers`)),r.addEventListener(`click`,e=>{e.target===r&&i(`cancel`)})})}function C(e,n){return e===`hybrid`||n.some(n=>{let r=t.has(n.adapterId);return e===`local`?r:!r&&(n.noKeyNeeded||!!(n.apiKey&&n.apiKey.trim()))})?{ready:!0}:{ready:!1,modeLabel:e===`cloud`?`Cloud Only`:`Local Only`}}var w=`magenais.consent.v1.`;function T(e){return`${w}${e}`}function E(e){try{if(typeof localStorage>`u`)return`UNKNOWN`;let t=localStorage.getItem(T(e));return t===`ACCEPTED`||t===`DECLINED`||t===`DOWNLOADING`||t===`READY`?t:`UNKNOWN`}catch{return`UNKNOWN`}}function D(e,t){try{if(typeof localStorage>`u`)return;t===`UNKNOWN`?localStorage.removeItem(T(e)):localStorage.setItem(T(e),t)}catch{}}var O={get(e){return E(e)},hasAnswered(e){return E(e)!==`UNKNOWN`},accept(e){D(e,`ACCEPTED`)},decline(e){D(e,`DECLINED`)},startDownload(e){D(e,`DOWNLOADING`)},markReady(e){D(e,`READY`)},downloadFailed(e){D(e,`ACCEPTED`)},reset(e){D(e,`UNKNOWN`)}},k=1e3,A=new Set([`person`,`bicycle`,`car`,`motorcycle`,`bus`,`truck`,`boat`,`train`,`dog`,`cat`]),j=class extends v{stream=null;videoEl=null;overlayEl=null;simCanvasEl=null;liveInterval=null;simDrawInterval=null;simulating=!1;simTick=0;analyzing=!1;selectedMode=`car`;events=[];messages=[];diagnostics=[];lastInferenceMs=null;frameTimestamps=[];speedKmh=0;headingDeg=0;steerDeg=0;gear=`D`;stopPct=0;throttlePct=0;muteVoice=!0;largeCaptions=!1;hardwareStatus=`No connection`;gpsStatus=`GPS: —`;modelBootstrap=null;loggedCocoSsdCooldownUntil=null;routingGapAcknowledged=!1;modelBootstrapYolos=null;async ensureYolosModelReady(){let e=this.kernel.getProviderManager().getProvider(`preset-transformers-autonomous`);if(!e||!e.enabled)return!1;let t=s(`object-detection`)||e.defaultModel||`Xenova/yolos-tiny`;if(r(t,`object-detection`))return!0;if(this.modelBootstrapYolos)return this.modelBootstrapYolos;let n=`preset-transformers-autonomous:${t}`;if(O.get(n)===`DECLINED`)return!1;let o=a(t)?.displayName||t.split(`/`).pop()||t;this.modelBootstrapYolos=(async()=>{if(!O.hasAnswered(n)){let{consented:e}=await x(o);if(!e)return O.decline(n),this.logDiagnostic(`Download declined — this won't be asked again. Analyze Frame will try any other configured provider instead. Open Local Models in the Universal Provider Manager any time to download "${o}".`),!1;O.accept(n)}O.startDownload(n);let e=document.getElementById(`autoAnalyzeBtn`);e&&(e.disabled=!0,e.textContent=`⬇ Downloading model — please wait…`),this.logDiagnostic(`⬇ Downloading "${o}" (on-device object detection) — one-time only, then cached by the browser for every future session. Please wait…`);try{return await i(t,`object-detection`,void 0,e=>this.logDiagnostic(e)),O.markReady(n),this.logDiagnostic(`✓ "${o}" ready — Autonomous tab is fully set up. Starting analysis…`),!0}catch(e){return O.downloadFailed(n),this.logDiagnostic(`✗ Download failed (${e?.message||e}). Open Local Models in the Universal Provider Manager to retry.`),!1}finally{this.syncLiveButtons()}})();try{return await this.modelBootstrapYolos}finally{this.modelBootstrapYolos=null}}activate(){this.renderOutputShell(),this.logDiagnostic(`OpenPilot dash initialized — built-in perception is ready now. Choose a mode, then enable the camera or a simulated feed to begin. If COCO-SSD or YOLOS/DETR is enabled and fully downloaded (Local Models), it's used automatically for real class-labeled detection (person, car, etc.); otherwise OpenPilot's built-in motion/edge engine covers you with zero setup.`),this.renderDash(),this.ensureOnDeviceConsentReady(),this.ensureDefaultModelReady(),this.ensureYolosModelReady()}async ensureDefaultModelReady(){let t=this.kernel.getProviderManager().getProvider(`builtin-transformers-autonomous`);if(t&&!t.enabled)return!1;if(o())return!0;if(l()){let e=f();return this.loggedCocoSsdCooldownUntil!==e&&(this.loggedCocoSsdCooldownUntil=e??null,this.logDiagnostic(`COCO-SSD is cooling down (${c()} left) after: ${m()||`a recent failure`}. Skipping retry until then — Analyze Frame will try any other configured provider instead (including the built-in OpenPilot engine, unaffected by this cooldown).`)),!1}if(this.loggedCocoSsdCooldownUntil=null,this.modelBootstrap)return this.modelBootstrap;let n=`builtin-transformers-autonomous`;if(O.get(n)===`DECLINED`)return!1;let r=p();this.modelBootstrap=(async()=>{let t=`COCO-SSD`;if(!r&&!O.hasAnswered(n)){let{consented:e}=await x(t);if(!e)return O.decline(n),this.logDiagnostic(`Download declined — this won't be asked again. Analyze Frame will try any other configured provider instead. Open Local Models in the Universal Provider Manager any time to download the default detector.`),!1;O.accept(n)}O.startDownload(n);let i=document.getElementById(`autoAnalyzeBtn`);i&&(i.disabled=!0,i.textContent=r?`⬇ Reloading model — please wait…`:`⬇ Downloading model — please wait…`),this.logDiagnostic(r?`Reloading "${t}" for this session (already installed on this device — no re-download, just the in-memory setup step). Please wait…`:`⬇ Downloading "${t}" (real class-labeled on-device object detection — a Downloadable alternative to the built-in OpenPilot engine) — one-time only (~6MB), then cached by the browser for every future session. This usually takes just a few seconds. Please wait…`);try{return await e(e=>this.logDiagnostic(e)),O.markReady(n),this.logDiagnostic(`✓ Object detection model ready — Autonomous tab is fully set up. Starting analysis…`),!0}catch(e){return O.downloadFailed(n),this.logDiagnostic(`✗ Download failed (${e?.message||e}). Open Local Models in the Universal Provider Manager to retry.`),!1}finally{this.syncLiveButtons()}})();try{return await this.modelBootstrap}finally{this.modelBootstrap=null}}dispatchActionCommand(e,t){let n=[`preset-openpilot-bridge`,`preset-robot-bridge`,`preset-drone-bridge`],r=this.kernel.getProviderManager();for(let i of n){let n=r.getProvider(i);if(!n||!n.enabled)continue;let a=`${n.baseUrl.replace(/\/$/,``)}/actions`,o=new AbortController,s=setTimeout(()=>o.abort(),3e3);fetch(a,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({command:e,reason:t,mode:this.selectedMode,timestamp:Date.now()}),signal:o.signal}).catch(e=>{console.debug(`[Autonomous] Action dispatch to "${n.name}" (${a}) failed — bridge unreachable or not implementing /actions yet: ${e?.message||e}`)}).finally(()=>clearTimeout(s))}}renderDash(){let e=g(this.selectedMode);this.renderControl(`
