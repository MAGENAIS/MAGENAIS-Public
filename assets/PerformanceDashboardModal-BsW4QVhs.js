import{M as e,R as t,et as n,j as r,k as i}from"./main-N2xr38iN.js";import{a,i as o,n as s,o as c,r as l,t as u}from"./PerformanceTelemetry-DqrxEkhL.js";function d(e){let t=document.createElement(`div`);return t.textContent=e??``,t.innerHTML}function f(e){if(!e)return`—`;let t=Math.floor((Date.now()-e)/1e3);if(t<5)return`just now`;if(t<60)return`${t}s ago`;let n=Math.floor(t/60);if(n<60)return`${n}m ago`;let r=Math.floor(n/60);return r<24?`${r}h ago`:`${Math.floor(r/24)}d ago`}function p(e){return e==null||Number.isNaN(e)?`—`:e<1e3?`${Math.round(e)}ms`:`${(e/1e3).toFixed(1)}s`}function m(e){return`${Math.round(e)}%`}function h(e,t){return e.length>t?`${e.slice(0,t-1)}…`:e}function g(e,t,n){let r=new Blob([t],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=e,document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(i),1e3)}var _={all:null,"15m":9e5,"1h":36e5,"24h":864e5},v=class v{kernel;unsubscribeEvents=null;filters={provider:`all`,type:`all`,status:`all`,timeRange:`all`};renderQueued=!1;constructor(e){this.kernel=e,o()}open(){this.ensureDom(),this.render(),document.getElementById(`performanceModal`)?.classList.add(`open`),this.unsubscribeEvents||=n.onEvent(()=>this.queueRender())}close(){document.getElementById(`performanceModal`)?.classList.remove(`open`),this.unsubscribeEvents&&=(this.unsubscribeEvents(),null)}queueRender(){this.renderQueued||(this.renderQueued=!0,requestAnimationFrame(()=>{this.renderQueued=!1,this.render()}))}ensureDom(){if(document.getElementById(`performanceModal`))return;let e=document.createElement(`div`);e.className=`modal-backdrop`,e.id=`performanceModal`,e.innerHTML=`
    ...
    `,document.body.appendChild(e),e.addEventListener(`click`,t=>{t.target===e&&this.close()}),document.getElementById(`closePerformance`)?.addEventListener(`click`,()=>this.close()),this.renderFilters()}renderFilters(){let e=document.getElementById(`perfFilters`);if(!e)return;let t=n.getRecentEvents(),r=Array.from(new Set(t.map(e=>e.provider))).sort(),i=Array.from(new Set(t.map(e=>e.providerType).filter(e=>!!e))).sort();e.innerHTML=`
      <select class="perf-filter-select" id="perfFilterProvider">
        <option value="all">All providers</option>
        ${r.map(e=>`<option value="${d(e)}" ${this.filters.provider===e?`selected`:``}>${d(e)}</option>`).join(``)}
      </select>
      <select class="perf-filter-select" id="perfFilterType">
        <option value="all">All modes</option>
        ${i.map(e=>`<option value="${d(e)}" ${this.filters.type===e?`selected`:``}>${d(e)}</option>`).join(``)}
      </select>
    ...
    `,this.fillSystemMetrics(),this.wireExpandableRows(),document.getElementById(`perfClearHistory`)?.addEventListener(`click`,()=>{s(),this.render()})}}renderKpis(e){return`<div class="perf-kpi-grid">${[[`🚀`,`Fastest Provider`,e.fastestProvider||`—`],[`🛡️`,`Most Reliable`,e.mostReliableProvider||`—`],[`⭐`,`Recommended`,e.recommendedProvider||`—`],[`📈`,`Avg Latency`,p(e.avgLatencyMs)],[`🔄`,`Success Rate`,m(e.successRate)],[`⚡`,`Active Providers`,String(e.activeProviders)]].map(([e,t,n])=>`
      ...
      </section>`}renderSystemMetricsPlaceholder(){return`<section class="perf-section" id="perfSystemMetrics"><h4>System Metrics</h4><p class="hint">Loading…</p></section>`}async fillSystemMetrics(){let t=document.getElementById(`perfSystemMetrics`);if(!t)return;let n=performance.memory?.usedJSHeapSize?`${(performance.memory.usedJSHeapSize/1048576).toFixed(0)}MB`:null,a=typeof navigator<`u`&&!!navigator.gpu,o=await e(),s=r(),c=Object.values(s).filter(e=>e.status===`ready`),l=c.reduce((e,t)=>e+(t.bytesTotal||t.bytesDownloaded||0),0);t.innerHTML=`<h4>System Metrics</h4><div class="perf-kv-grid">${[[`Memory Usage`,n?`${n} (JS heap, approximate)`:`Not exposed by this browser`],[`WebGPU Status`,a?`Available`:`Not available`],[`Cache Size`,o.supported?i(o.usageBytes):`Not exposed by this browser`],[`Downloaded Models`,String(c.length)],[`Local Model Size`,c.length>0?i(l):`—`]].map(([e,t])=>`<div><span class="perf-kv-label">${d(e)}</span>${d(t)}</div>`).join(``)}</div>`}renderCharts(e,t,n,r){if(e.length===0)return``;let i=e.reduce((e,t)=>e+t.successCount,0),a=e.reduce((e,t)=>e+t.failureCount,0),o=r.filter(e=>e.winner).slice(0,12).reverse(),s=[...e].filter(e=>e.totalRequests>0).sort((e,t)=>t.successRate-e.successRate).slice(0,6);return`
      <section class="perf-section">
    ...
    `).join(``);return`<div class="perf-chart-box"><div class="perf-chart-title">${d(t)}</div><svg viewBox="0 0 240 150" class="perf-chart-svg perf-chart-svg-tall"><line x1="10" y1="128" x2="230" y2="128" class="perf-chart-baseline"/><polyline points="${a}" fill="none" stroke="var(--azure)" stroke-width="1.5"/>${s}</svg></div>`}svgHistogram(e,t){if(e.length===0)return`<div class="perf-chart-box"><div class="perf-chart-title">${d(t)}</div><p class="hint">No latency data yet.</p></div>`;let n=Math.min(6,Math.max(3,Math.ceil(Math.sqrt(e.length)))),r=Math.min(...e),i=Math.max(...e),a=Math.max(1,(i-r)/n),o=Array(n).fill(0);for(let t of e){let e=Math.min(n-1,Math.floor((t-r)/a));o[e]++}let s=v.BAR_3D_DX,c=-v.BAR_3D_DY,l=s+4,u=126-(14+c),f=Math.max(1,...o),p=(236-l)/n,m=Math.max(2,p-6),h=o.map((e,t)=>{let n=e>0?Math.max(2,e/f*u):0,i=4+t*p+(p-m)/2,o=126-n,s=Math.round(r+t*a),l=Math.round(r+(t+1)*a),d=`${s}-${l}`;return`
      ...
      </section>`:``}naSpan(){return`<span class="perf-na">Not Available</span>`}inspectorKv(e){return`<div class="perf-kv-grid">${e.map(([e,t])=>`<div><span class="perf-kv-label">${d(e)}</span>${t==null||t===``?this.naSpan():d(String(t))}</div>`).join(``)}</div>`}friendlyModelType(e){return e&&{text:`LLM`,coding:`LLM (Code)`,agents:`LLM (Agent)`,research:`LLM (Research)`,vision:`Vision`,image:`Image Generation`,video:`Video Generation`,speech:`Audio (TTS)`,audio:`Audio (STT)`,music:`Audio (Music)`,embeddings:`Embedding`,autonomous:`Vision (Object Detection)`,gamegen:`LLM (Code)`}[e]||e||`Unknown`}friendlyApiCompat(e){return e?e===`openai-compatible`||e===`openrouter`||e===`groq`?`OpenAI Compatible`:e===`transformers`||e===`webllm`?`Local Runtime (in-browser)`:e===`ollama`?`Local Runtime (Ollama server)`:`Native API`:`Unknown`}renderModelInspector(e){if(!e||e.attempts.length===0)return``;let n=e.winner||e.attempts[e.attempts.length-1],a=this.kernel.getProviderManager().getProviders().find(e=>e.name===n.provider),o=a?.adapterId===`transformers`||a?.adapterId===`webllm`,s=n.model?t(n.model):void 0,c=n.model?Object.values(r()).find(e=>e.modelId===n.model):void 0,l=n.timestamp-(n.latencyMs||0),u=n.tokens?.completion&&n.latencyMs?(n.tokens.completion/(n.latencyMs/1e3)).toFixed(1):void 0,f=n.params?.contextWindow??s?.contextLength,p=n.tokens?.total??(n.tokens?.prompt!==void 0&&n.tokens?.completion!==void 0?n.tokens.prompt+n.tokens.completion:void 0),m=f&&p?`${Math.min(100,Math.round(p/f*100))}%`:void 0;return`
      <section class="perf-section">
        <h4>Model Parameter Inspector <span class="hint">(${e.winner?`pipeline winner`:`most recent attempt — no winner yet`} — ${d(n.provider)}${n.model?` / ${d(n.model)}`:``})</span></h4>

        <details class="perf-inspector-details" open>
          <summary>Model Overview</summary>
          ${this.inspectorKv([[`Provider`,n.provider],[`Model Name`,n.model],[`Model ID`,n.model],[`Model Family`,s?.family],[`Model Version`,s?.version],[`Model Type`,this.friendlyModelType(n.providerType)],[`Quantization`,s?.quantization],[`Backend Compatibility`,s?.backendCompatibility],[`Supported Languages`,s?.supportedLanguages?.join(`, `)],[`Release Date`,void 0],[`Parameter Count`,void 0],[`Architecture`,void 0],[`Fine-tuning Info`,void 0],[`Precision`,void 0]])}
        </details>
        ...
        <details class="perf-inspector-details">
          ...
          ${this.inspectorKv(n.params&&Object.keys(n.params).length>0?Object.entries(n.params).map(([e,t])=>[e,String(t)]):[[`Parameters`,void 0]])}
        ...
        <details class="perf-inspector-details">
          ...
          ${this.inspectorKv([[`Max Context Window`,f],[`Input Tokens`,n.tokens?.prompt],[`Output Tokens`,n.tokens?.completion],[`Total Tokens`,p],[`Context Utilization`,m],[`System/User/Tool Token Breakdown`,void 0]])}
        </details>

        <details class="perf-inspector-details">
          <summary>Capabilities</summary>
          ${this.inspectorKv([[`Vision-only Provider`,a?.visionOnly?`Yes`:a?`No`:void 0],[`Declared Capabilities`,a?.capabilities?.join(`, `)],[`Local Model Capabilities`,s?.capabilities?.join(`, `)],[`Streaming (this request)`,n.params?.stream===void 0?void 0:String(n.params.stream)],[`JSON Mode (this request)`,n.params?.jsonMode===void 0?void 0:String(n.params.jsonMode)],[`Function Calling (this request)`,n.params?.functionCalling===void 0?void 0:String(n.params.functionCalling)],[`Thinking/Reasoning (this request)`,n.params?.thinking===void 0?n.params?.reasoning===void 0?void 0:String(n.params.reasoning):String(n.params.thinking)]])}
        </details>

        <details class="perf-inspector-details">
          <summary>API Request Information</summary>
          ${this.inspectorKv([[`API Compatibility`,this.friendlyApiCompat(a?.adapterId)],[`Streaming Status`,n.params?.stream===void 0?void 0:String(n.params.stream)],[`Request Timestamp`,new Date(l).toLocaleString()],[`Response Timestamp`,new Date(n.timestamp).toLocaleString()],[`Total Latency`,`${Math.round(n.latencyMs||0)}ms`],[`Time to First Token`,void 0],[`Generation Speed`,u?`${u} tokens/sec`:void 0],[`Retry Count`,n.retryCount],[`Winner (this pipeline)`,n.winner?`Yes`:`No`]])}
        </details>
        ...
        <details class="perf-inspector-details">
          ...
          ${this.inspectorKv([[`Runtime`,a?.adapterId===`webllm`?`WebLLM`:`Transformers.js (ONNX Runtime Web)`],[`Backend`,c?.device===`webgpu`?`WebGPU`:c?.device===`wasm`?`WASM`:void 0],[`Download Size`,c?.bytesTotal?i(c.bytesTotal):s?.downloadSizeMB?`~${s.downloadSizeMB}MB`:void 0],[`Cached/Downloaded Size`,c?.bytesDownloaded?i(c.bytesDownloaded):void 0],[`Loaded Status`,c?.status],[`Approx. RAM Requirement`,s?.ramRequirementMB?`~${s.ramRequirementMB}MB`:void 0],[`Memory Usage (JS heap, whole page)`,performance.memory?.usedJSHeapSize?i(performance.memory.usedJSHeapSize):void 0],[`Loading Time`,void 0]])}
        </details>`:``}
        ...
        <details class="perf-inspector-details">
          ...
          <div class="perf-trace">
            ...
            <div class="perf-trace-step"><span class="perf-trace-dot"></span>Provider Selected — ${d(n.provider)}</div>
            <div class="perf-trace-line"></div>
            ...
            <div class="perf-trace-line"></div>
            <div class="perf-trace-step ${n.failureReason?`perf-trace-error`:`perf-trace-ok`}"><span class="perf-trace-dot"></span>${n.failureReason?`Failed — ${d(n.failureReason)}`:`Generation Completed`} — ${new Date(n.timestamp).toLocaleTimeString()} (${Math.round(n.latencyMs||0)}ms total)</div>
          </div>
        </details>
        ...
        <details class="perf-inspector-details">
          ...
          <pre class="perf-raw-json">${d(JSON.stringify(n,null,2))}</pre>
