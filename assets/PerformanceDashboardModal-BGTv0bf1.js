    ...
    `).join(``);return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><svg viewBox="0 0 240 150" class="perf-chart-svg perf-chart-svg-tall"><line x1="10" y1="128" x2="230" y2="128" class="perf-chart-baseline"/><polyline points="${o}" fill="none" stroke="var(--azure)" stroke-width="1.5"/>${c}</svg></div>`}svgHistogram(e,t){if(e.length===0)return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><p class="hint">No latency data yet.</p></div>`;let n=Math.min(6,Math.max(3,Math.ceil(Math.sqrt(e.length)))),r=Math.min(...e),i=Math.max(...e),o=Math.max(1,(i-r)/n),s=Array(n).fill(0);for(let t of e){let e=Math.min(n-1,Math.floor((t-r)/o));s[e]++}let c=v.BAR_3D_DX,l=-v.BAR_3D_DY,u=c+4,d=126-(14+l),f=Math.max(1,...s),p=(236-u)/n,m=Math.max(2,p-6),h=s.map((e,t)=>{let n=e>0?Math.max(2,e/f*d):0,i=4+t*p+(p-m)/2,a=126-n,s=Math.round(r+t*o),c=Math.round(r+(t+1)*o),u=`${s}-${c}`;return`
      ...
      <section class="perf-section">
        ...
        <details class="perf-inspector-details">
          ...
          ${this.inspectorKv([[`Runtime`,o?.adapterId===`webllm`?`WebLLM`:`Transformers.js (ONNX Runtime Web)`],[`Backend`,l?.device===`webgpu`?`WebGPU`:l?.device===`wasm`?`WASM`:void 0],[`Download Size`,l?.bytesTotal?t(l.bytesTotal):c?.downloadSizeMB?`~${c.downloadSizeMB}MB`:void 0],[`Cached/Downloaded Size`,l?.bytesDownloaded?t(l.bytesDownloaded):void 0],[`Loaded Status`,l?.status],[`Approx. RAM Requirement`,c?.ramRequirementMB?`~${c.ramRequirementMB}MB`:void 0],[`Memory Usage (JS heap, whole page)`,performance.memory?.usedJSHeapSize?t(performance.memory.usedJSHeapSize):void 0],[`Loading Time`,void 0]])}
        </details>`:``}
