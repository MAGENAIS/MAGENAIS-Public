import{o as e}from"./main-C8vn95zW.js";import{t}from"./Mode-DuooQ1Rf.js";var n=[{key:`semanticscholar`,label:`Semantic Scholar`},{key:`openalex`,label:`OpenAlex`},{key:`arxiv`,label:`arXiv`}],r=class extends t{activate(){this.renderControl(`
      <div class="field">
    ...
    `),document.querySelectorAll(`#researchSourceChips .chip`).forEach(e=>{e.addEventListener(`click`,()=>{let t=document.querySelectorAll(`#researchSourceChips .chip.active`).length;e.classList.contains(`active`)&&t<=1||e.classList.toggle(`active`)})});let e=document.getElementById(`researchCount`);e?.addEventListener(`input`,()=>{let t=document.getElementById(`researchCountVal`);t&&(t.textContent=e.value)}),document.getElementById(`runBtn`)?.addEventListener(`click`,()=>this.runGuarded(`runBtn`,()=>this.handleRun()))}async handleRun(){let e=document.getElementById(`promptInput`)?.value.trim();if(!e){alert(`Enter a research question.`);return}let t=Array.from(document.querySelectorAll(`#researchSourceChips .chip.active`)).map(e=>e.dataset.val).filter(Boolean),n=parseInt(document.getElementById(`researchCount`)?.value||`6`,10);this.outputPanel.querySelector(`.stage`)&&this.renderLoading(`Gathering sources…`);let r={id:`research-`+Date.now(),name:`Research`,graph:{nodes:[{id:`research1`,type:`research`,label:`Research Agent`,config:{sources:t,limitPerSource:n},inputs:{query:e},enabled:!0}],edges:[]},createdAt:Date.now(),updatedAt:Date.now()};try{let t=await this.kernel.getWorkflowEngine().execute(r,{query:e},(e,t)=>this.appendLog(e,t));this.renderResult(t.finalOutput),this.kernel.getStore().getActions().addHistoryEntry({mode:`research`,prompt:e,result:t.finalOutput?.summary,resultType:`text`})}catch(e){this.renderError(e)}}renderResult(t){let n=this.outputPanel.querySelector(`.stage`);if(!n)return;let{papers:r,sourceStatus:i,summary:a}=t||{},o=``;i&&(o+=`<div class="chip-group" style="margin-bottom:12px;">${i.map(e=>`<span class="chip ${e.status===`ok`?`active`:``}" style="${e.status===`fail`?`opacity:.5;`:``}">${e.name} ${e.status===`ok`?`✓`:`✕`}</span>`).join(``)}</div>`),a&&(o+=`<div class="doc-summary-block" style="margin-bottom:18px;"><div class="result-text">${this.renderMarkdown(a)}</div>${this.renderReadAloudBlock(e(a),`Read Summary Aloud`)}</div>`),r?.length&&(o+=`<p class="field-label">Sources (${r.length})</p>`,o+=r.slice(0,20).map(e=>`
      ...
      `).join(``)),n.innerHTML=o||`<div class="empty-text">No results.</div>`,this.wireReadAloudControls(),this.wireCodeCopyButtons(n)}deactivate(){typeof window<`u`&&window.speechSynthesis&&window.speechSynthesis.cancel()}getTitle(){return`Research`}};export{r as ResearchMode};