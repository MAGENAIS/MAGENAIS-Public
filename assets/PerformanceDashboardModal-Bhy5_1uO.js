import{I as e,L as t,P as n,U as r,it as i}from"./main-DOB2GXWU.js";import{n as a}from"./RenderMarkdown-BLhzDcvV.js";import{a as o,i as s,n as c,o as l,r as u,t as d}from"./PerformanceTelemetry-CrtHsTl-.js";function f(e){if(!e)return`—`;let t=Math.floor((Date.now()-e)/1e3);if(t<5)return`just now`;if(t<60)return`${t}s ago`;let n=Math.floor(t/60);if(n<60)return`${n}m ago`;let r=Math.floor(n/60);return r<24?`${r}h ago`:`${Math.floor(r/24)}d ago`}function p(e){return e==null||Number.isNaN(e)?`—`:e<1e3?`${Math.round(e)}ms`:`${(e/1e3).toFixed(1)}s`}function m(e){return`${Math.round(e)}%`}function h(e,t){return e.length>t?`${e.slice(0,t-1)}…`:e}function g(e,t,n){let r=new Blob([t],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=e,document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(i),1e3)}var _={all:null,"15m":9e5,"1h":36e5,"24h":864e5},v=class v{kernel;unsubscribeEvents=null;filters={provider:`all`,type:`all`,status:`all`,timeRange:`all`};renderQueued=!1;constructor(e){this.kernel=e,s()}open(){this.ensureDom(),this.render(),document.getElementById(`performanceModal`)?.classList.add(`open`),this.unsubscribeEvents||=i.onEvent(()=>this.queueRender())}close(){document.getElementById(`performanceModal`)?.classList.remove(`open`),this.unsubscribeEvents&&=(this.unsubscribeEvents(),null)}queueRender(){this.renderQueued||(this.renderQueued=!0,requestAnimationFrame(()=>{this.renderQueued=!1,this.render()}))}ensureDom(){if(document.getElementById(`performanceModal`))return;let e=document.createElement(`div`);e.className=`modal-backdrop`,e.id=`performanceModal`,e.innerHTML=`
    ...
    `,document.body.appendChild(e),e.addEventListener(`click`,t=>{t.target===e&&this.close()}),document.getElementById(`closePerformance`)?.addEventListener(`click`,()=>this.close()),this.renderFilters()}renderFilters(){let e=document.getElementById(`perfFilters`);if(!e)return;let t=i.getRecentEvents(),n=Array.from(new Set(t.map(e=>e.provider))).sort(),r=Array.from(new Set(t.map(e=>e.providerType).filter(e=>!!e))).sort();e.innerHTML=`
      <select class="perf-filter-select" id="perfFilterProvider">
        ...
        ${n.map(e=>`<option value="${a(e)}" ${this.filters.provider===e?`selected`:``}>${a(e)}</option>`).join(``)}
      </select>
      <select class="perf-filter-select" id="perfFilterType">
        <option value="all">All modes</option>
        ${r.map(e=>`<option value="${a(e)}" ${this.filters.type===e?`selected`:``}>${a(e)}</option>`).join(``)}
      </select>
    ...
    `,this.fillSystemMetrics(),this.wireExpandableRows(),document.getElementById(`perfClearHistory`)?.addEventListener(`click`,()=>{c(),this.render()})}}renderKpis(e){return`<div class="perf-kpi-grid">${[[`🚀`,`Fastest Provider`,e.fastestProvider||`—`],[`🛡️`,`Most Reliable`,e.mostReliableProvider||`—`],[`⭐`,`Recommended`,e.recommendedProvider||`—`],[`📈`,`Avg Latency`,p(e.avgLatencyMs)],[`🔄`,`Success Rate`,m(e.successRate)],[`⚡`,`Active Providers`,String(e.activeProviders)]].map(([e,t,n])=>`
      ...
      <div class="perf-bar-row" title="${a(e.provider)}: ${p(e.latencyMs)}">
        <span class="perf-bar-label">${a(e.provider)}</span>
      ...
      <section class="perf-section">
        <h4>Pipeline Summary <span class="perf-status-pill" style="color:${t}; border-color:${t};">${a(e.status)}</span></h4>
        <div class="perf-kv-grid">
          <div><span class="perf-kv-label">Winner</span>${a(e.winner?.provider||`—`)}</div>
          <div><span class="perf-kv-label">Model</span>${a(e.winner?.model||`—`)}</div>
      ...
      <section class="perf-section">
        ...
        <div class="perf-kv-grid">${n.map(([e,t])=>`<div><span class="perf-kv-label">${a(e)}</span>${a(t)}</div>`).join(``)}</div>
        <p class="hint">Only parameters this specific request actually reported — nothing here is estimated.</p>
      </section>`}renderSystemMetricsPlaceholder(){return`<section class="perf-section" id="perfSystemMetrics"><h4>System Metrics</h4><p class="hint">Loading…</p></section>`}async fillSystemMetrics(){let r=document.getElementById(`perfSystemMetrics`);if(!r)return;let i=performance.memory?.usedJSHeapSize?`${(performance.memory.usedJSHeapSize/1048576).toFixed(0)}MB`:null,o=typeof navigator<`u`&&!!navigator.gpu,s=await t(),c=e(),l=Object.values(c).filter(e=>e.status===`ready`),u=l.reduce((e,t)=>e+(t.bytesTotal||t.bytesDownloaded||0),0);r.innerHTML=`<h4>System Metrics</h4><div class="perf-kv-grid">${[[`Memory Usage`,i?`${i} (JS heap, approximate)`:`Not exposed by this browser`],[`WebGPU Status`,o?`Available`:`Not available`],[`Cache Size`,s.supported?n(s.usageBytes):`Not exposed by this browser`],[`Downloaded Models`,String(l.length)],[`Local Model Size`,l.length>0?n(u):`—`]].map(([e,t])=>`<div><span class="perf-kv-label">${a(e)}</span>${a(t)}</div>`).join(``)}</div>`}renderCharts(e,t,n,r){if(e.length===0)return``;let i=e.reduce((e,t)=>e+t.successCount,0),a=e.reduce((e,t)=>e+t.failureCount,0),o=r.filter(e=>e.winner).slice(0,12).reverse(),s=[...e].filter(e=>e.totalRequests>0).sort((e,t)=>t.successRate-e.successRate).slice(0,6);return`
      <section class="perf-section">
      ...
      </section>`}static BAR_3D_DX=3;static BAR_3D_DY=-4;bar3d(e,t,n,r,i,a){let o=v.BAR_3D_DX,s=v.BAR_3D_DY,c=`<polygon points="${e},${t} ${e+o},${t+s} ${e+n+o},${t+s} ${e+n},${t}" fill="${i}" opacity="0.85"/>`;return`<g><title>${a}</title>${`<polygon points="${e+n},${t} ${e+n+o},${t+s} ${e+n+o},${t+r+s} ${e+n},${t+r}" fill="${i}" opacity="0.55"/>`}${c}${`<rect x="${e}" y="${t}" width="${n}" height="${r}" fill="${i}"/>`}</g>`}svgBarChart(e,t,n=`ms`){if(e.length===0)return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><p class="hint">No pipeline yet.</p></div>`;let r=v.BAR_3D_DX,i=-v.BAR_3D_DY,o=r+4,s=14+i,c=Math.max(1,...e.map(e=>e.value)),l=126-s,u=(236-o)/e.length,d=Math.max(2,u-10),f=e.map((e,t)=>{let r=Math.max(2,e.value/c*l),o=4+t*u+(u-d)/2,s=126-r,f=`${Math.round(e.value)}${n}`;return`
        ${this.bar3d(o,s,d,r,`var(--amber)`,`${a(e.label)}: ${f}`)}
        <text x="${o+d/2}" y="${s-i-3}" text-anchor="middle" class="perf-chart-value-label">${a(f)}</text>
        <text x="${o+d/2}" y="144" text-anchor="middle" class="perf-chart-axis-label">${a(h(e.label,10))}</text>`}).join(``);return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><svg viewBox="0 0 240 150" class="perf-chart-svg perf-chart-svg-tall"><line x1="4" y1="126" x2="${240-o+r}" y2="126" class="perf-chart-baseline"/>${f}</svg></div>`}svgLineChart(e,t){if(e.length<2)return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><p class="hint">Needs at least 2 completed pipelines.</p></div>`;let n=Math.max(1,...e.map(e=>e.value)),r=220/(e.length-1),i=e.map((e,t)=>({x:10+t*r,y:128-e.value/n*112,d:e})),o=i.map(e=>`${e.x.toFixed(1)},${e.y.toFixed(1)}`).join(` `),s=Math.ceil(e.length/6),c=i.map((e,t)=>`
      <circle cx="${e.x.toFixed(1)}" cy="${e.y.toFixed(1)}" r="2.5" fill="var(--azure)"><title>${a(e.d.label)}: ${Math.round(e.d.value)}ms</title></circle>
      ${t%s===0||t===i.length-1?`<text x="${e.x.toFixed(1)}" y="${(e.y-6).toFixed(1)}" text-anchor="middle" class="perf-chart-value-label">${Math.round(e.d.value)}</text>`:``}
      ${t%s===0||t===i.length-1?`<text x="${e.x.toFixed(1)}" y="144" text-anchor="middle" class="perf-chart-axis-label">${a(e.d.label)}</text>`:``}
    `).join(``);return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><svg viewBox="0 0 240 150" class="perf-chart-svg perf-chart-svg-tall"><line x1="10" y1="128" x2="230" y2="128" class="perf-chart-baseline"/><polyline points="${o}" fill="none" stroke="var(--azure)" stroke-width="1.5"/>${c}</svg></div>`}svgHistogram(e,t){if(e.length===0)return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><p class="hint">No latency data yet.</p></div>`;let n=Math.min(6,Math.max(3,Math.ceil(Math.sqrt(e.length)))),r=Math.min(...e),i=Math.max(...e),o=Math.max(1,(i-r)/n),s=Array(n).fill(0);for(let t of e){let e=Math.min(n-1,Math.floor((t-r)/o));s[e]++}let c=v.BAR_3D_DX,l=-v.BAR_3D_DY,u=c+4,d=126-(14+l),f=Math.max(1,...s),p=(236-u)/n,m=Math.max(2,p-6),h=s.map((e,t)=>{let n=e>0?Math.max(2,e/f*d):0,i=4+t*p+(p-m)/2,a=126-n,s=Math.round(r+t*o),c=Math.round(r+(t+1)*o),u=`${s}-${c}`;return`
        ${e>0?this.bar3d(i,a,m,n,`var(--moss)`,`${s}-${c}ms: ${e} call(s)`):``}
        ${e>0?`<text x="${i+m/2}" y="${a-l-3}" text-anchor="middle" class="perf-chart-value-label">${e}</text>`:``}
        <text x="${i+m/2}" y="144" text-anchor="middle" class="perf-chart-axis-label">${u}</text>`}).join(``);return`<div class="perf-chart-box"><div class="perf-chart-title">${a(t)}</div><svg viewBox="0 0 240 150" class="perf-chart-svg perf-chart-svg-tall"><line x1="4" y1="126" x2="${240-u+c}" y2="126" class="perf-chart-baseline"/>${h}</svg><div class="hint" style="font-size:9.5px;">ms per bucket</div></div>`}svgRankingChart(e){return e.length===0?`<div class="perf-chart-box"><div class="perf-chart-title">Provider Ranking</div><p class="hint">No data yet.</p></div>`:`<div class="perf-chart-box" style="flex-basis:100%; align-items:stretch;"><div class="perf-chart-title">Provider Ranking (by success rate)</div><div class="perf-bars">${e.map(e=>`
      <div class="perf-bar-row" title="${a(e.provider)}: ${m(e.successRate)} success">
        <span class="perf-bar-label">${a(e.provider)}</span>
      ...
      </div>`).join(``)}</div></div>`}svgDonutWithCenterKpi(e,t,n,r){let i=t.reduce((e,t)=>e+t.value,0);if(i<=0)return`<div class="perf-chart-box"><div class="perf-chart-title">${a(e)}</div><p class="hint">No data yet.</p></div>`;let o=2*Math.PI*40,s=0,c=t.filter(e=>e.value>0).map(e=>{let t=e.value/i*o,n=`<circle cx="50" cy="50" r="40" fill="none" stroke="${e.color}" stroke-width="14" stroke-dasharray="${t.toFixed(2)} ${(o-t).toFixed(2)}" stroke-dashoffset="${(-s).toFixed(2)}" transform="rotate(-90 50 50)"><title>${a(e.label)}: ${e.value}</title></circle>`;return s+=t,n}).join(``),l=t.filter(e=>e.value>0).map(e=>`<div class="perf-legend-item"><span class="perf-legend-dot" style="background:${e.color};"></span>${a(e.label)} (${e.value})</div>`).join(``);return`<div class="perf-chart-box">
      <div class="perf-chart-title">${a(e)}</div>
      <div class="perf-donut-kpi-wrap">
        ...
        <div class="perf-donut-kpi-center">
          <div class="perf-donut-kpi-value">${a(n)}</div>
          <div class="perf-donut-kpi-label">${a(r)}</div>
        </div>
      </div>
      <div class="perf-legend">${l}</div>
      ...
      <tr class="perf-expand-body" id="perf-expand-${e.requestId}-${n}" style="display:none;">
        <td colspan="9">
          <div class="perf-kv-grid">
            <div><span class="perf-kv-label">Request ID</span>${a(t.requestId)}</div>
            <div><span class="perf-kv-label">Pipeline ID</span>${a(e.requestId)}</div>
            <div><span class="perf-kv-label">Execution Order</span>${n+1} of ${e.attempts.length}</div>
            <div><span class="perf-kv-label">Provider Type</span>${a(t.providerType||`—`)}</div>
            ...
            <div><span class="perf-kv-label">Failure Reason</span>${a(t.failureReason||`—`)}</div>
            ${t.params?Object.entries(t.params).map(([e,t])=>`<div><span class="perf-kv-label">${a(e)}</span>${a(String(t))}</div>`).join(``):``}
          </div>
        </td>
      </tr>`).join(``)}</tbody>
          </table>
        </div>
      </section>`:``}naSpan(){return`<span class="perf-na">Not Available</span>`}inspectorKv(e){return`<div class="perf-kv-grid">${e.map(([e,t])=>`<div><span class="perf-kv-label">${a(e)}</span>${t==null||t===``?this.naSpan():a(String(t))}</div>`).join(``)}</div>`}friendlyModelType(e){return e&&{text:`LLM`,coding:`LLM (Code)`,agents:`LLM (Agent)`,research:`LLM (Research)`,vision:`Vision`,image:`Image Generation`,video:`Video Generation`,speech:`Audio (TTS)`,audio:`Audio (STT)`,music:`Audio (Music)`,embeddings:`Embedding`,autonomous:`Vision (Object Detection)`,gamegen:`LLM (Code)`}[e]||e||`Unknown`}friendlyApiCompat(e){return e?e===`openai-compatible`||e===`openrouter`||e===`groq`?`OpenAI Compatible`:e===`transformers`||e===`webllm`?`Local Runtime (in-browser)`:e===`ollama`?`Local Runtime (Ollama server)`:`Native API`:`Unknown`}renderModelInspector(t){if(!t||t.attempts.length===0)return``;let i=t.winner||t.attempts[t.attempts.length-1],o=this.kernel.getProviderManager().getProviders().find(e=>e.name===i.provider),s=o?.adapterId===`transformers`||o?.adapterId===`webllm`,c=i.model?r(i.model):void 0,l=i.model?Object.values(e()).find(e=>e.modelId===i.model):void 0,u=i.timestamp-(i.latencyMs||0),d=i.tokens?.completion&&i.latencyMs?(i.tokens.completion/(i.latencyMs/1e3)).toFixed(1):void 0,f=i.params?.contextWindow??c?.contextLength,p=i.tokens?.total??(i.tokens?.prompt!==void 0&&i.tokens?.completion!==void 0?i.tokens.prompt+i.tokens.completion:void 0),m=f&&p?`${Math.min(100,Math.round(p/f*100))}%`:void 0;return`
      <section class="perf-section">
        <h4>Model Parameter Inspector <span class="hint">(${t.winner?`pipeline winner`:`most recent attempt — no winner yet`} — ${a(i.provider)}${i.model?` / ${a(i.model)}`:``})</span></h4>

        <details class="perf-inspector-details" open>
          <summary>Model Overview</summary>
          ${this.inspectorKv([[`Provider`,i.provider],[`Model Name`,i.model],[`Model ID`,i.model],[`Model Family`,c?.family],[`Model Version`,c?.version],[`Model Type`,this.friendlyModelType(i.providerType)],[`Quantization`,c?.quantization],[`Backend Compatibility`,c?.backendCompatibility],[`Supported Languages`,c?.supportedLanguages?.join(`, `)],[`Release Date`,void 0],[`Parameter Count`,void 0],[`Architecture`,void 0],[`Fine-tuning Info`,void 0],[`Precision`,void 0]])}
        </details>

        ${t.attempts.length>1?`
        <details class="perf-inspector-details">
          <summary>Competing Models (${t.attempts.length} in this pipeline)</summary>
          ...
          ${t.attempts.map(e=>`
            <div class="perf-competing-model ${e.winner?`perf-competing-model-winner`:``}">
              <div class="perf-competing-model-header">
                ${e.winner?`🏆 `:``}<strong>${a(e.provider)}</strong>${e.model?` / ${a(e.model)}`:``}
        ...
        <details class="perf-inspector-details">
          ...
          ${this.inspectorKv([[`Vision-only Provider`,o?.visionOnly?`Yes`:o?`No`:void 0],[`Declared Capabilities`,o?.capabilities?.join(`, `)],[`Local Model Capabilities`,c?.capabilities?.join(`, `)],[`Streaming (this request)`,i.params?.stream===void 0?void 0:String(i.params.stream)],[`JSON Mode (this request)`,i.params?.jsonMode===void 0?void 0:String(i.params.jsonMode)],[`Function Calling (this request)`,i.params?.functionCalling===void 0?void 0:String(i.params.functionCalling)],[`Thinking/Reasoning (this request)`,i.params?.thinking===void 0?i.params?.reasoning===void 0?void 0:String(i.params.reasoning):String(i.params.thinking)]])}
        </details>

        <details class="perf-inspector-details">
          <summary>API Request Information</summary>
          ${this.inspectorKv([[`API Compatibility`,this.friendlyApiCompat(o?.adapterId)],[`Streaming Status`,i.params?.stream===void 0?void 0:String(i.params.stream)],[`Request Timestamp`,new Date(u).toLocaleString()],[`Response Timestamp`,new Date(i.timestamp).toLocaleString()],[`Total Latency`,`${Math.round(i.latencyMs||0)}ms`],[`Time to First Token`,void 0],[`Generation Speed`,d?`${d} tokens/sec`:void 0],[`Retry Count`,i.retryCount],[`Winner (this pipeline)`,i.winner?`Yes`:`No`]])}
        </details>

        <details class="perf-inspector-details">
          <summary>Provider Configuration</summary>
          ${this.inspectorKv([[`Provider Name`,o?.name],[`Endpoint`,o?.baseUrl],[`Auth Status`,o?o.noKeyNeeded?`No key required`:o.apiKey?`Key configured`:`Key missing`:void 0],[`Default Model`,o?.defaultModel],[`Priority`,o?.priority],[`Requires Backend Proxy`,o?o.requiresServerProxy?`Yes`:`No`:void 0],[`Rate Limits`,void 0],[`Remaining Quota`,void 0]])}
        </details>

        ${s?`
        <details class="perf-inspector-details">
          <summary>Local AI Runtime Information</summary>
          ${this.inspectorKv([[`Runtime`,o?.adapterId===`webllm`?`WebLLM`:`Transformers.js (ONNX Runtime Web)`],[`Backend`,l?.device===`webgpu`?`WebGPU`:l?.device===`wasm`?`WASM`:void 0],[`Download Size`,l?.bytesTotal?n(l.bytesTotal):c?.downloadSizeMB?`~${c.downloadSizeMB}MB`:void 0],[`Cached/Downloaded Size`,l?.bytesDownloaded?n(l.bytesDownloaded):void 0],[`Loaded Status`,l?.status],[`Approx. RAM Requirement`,c?.ramRequirementMB?`~${c.ramRequirementMB}MB`:void 0],[`Memory Usage (JS heap, whole page)`,performance.memory?.usedJSHeapSize?n(performance.memory.usedJSHeapSize):void 0],[`Loading Time`,void 0]])}
        </details>`:``}

        <details class="perf-inspector-details">
          <summary>Execution Trace</summary>
          <div class="perf-trace">
            <div class="perf-trace-step"><span class="perf-trace-dot"></span>Request Created — ${new Date(u).toLocaleTimeString()}</div>
            <div class="perf-trace-line"></div>
            <div class="perf-trace-step"><span class="perf-trace-dot"></span>Provider Selected — ${a(i.provider)}</div>
            <div class="perf-trace-line"></div>
            ...
            <div class="perf-trace-line"></div>
            <div class="perf-trace-step ${i.failureReason?`perf-trace-error`:`perf-trace-ok`}"><span class="perf-trace-dot"></span>${i.failureReason?`Failed — ${a(i.failureReason)}`:`Generation Completed`} — ${new Date(i.timestamp).toLocaleTimeString()} (${Math.round(i.latencyMs||0)}ms total)</div>
          </div>
        </details>
        ...
        <details class="perf-inspector-details">
          ...
          <pre class="perf-raw-json">${a(JSON.stringify(i,null,2))}</pre>
          <div class="hint">Matching provider configuration (API key redacted):</div>
          <pre class="perf-raw-json">${a(JSON.stringify(o?{...o,apiKey:o.apiKey?`[redacted]`:void 0}:null,null,2))}</pre>
        </details>
        ...
        <span class="perf-feed-provider">${a(e.provider)}</span>
        <span class="hint">${a(e.model||``)}</span>
        <span class="perf-feed-stage">${a(e.providerType||``)}</span>
        <span class="perf-feed-event">${a(t)}${e.failureReason?`: ${a(e.failureReason)}`:``}</span>
      </div>`}).join(``)}</div></section>`}renderSessionHistory(){let e=[...o()].reverse().slice(0,30);return e.length===0?``:`
      <section class="perf-section">
