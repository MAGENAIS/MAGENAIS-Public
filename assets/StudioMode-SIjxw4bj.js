      ...
      <div class="studio-control">
        ...
        <div class="studio-section">
          ...
          <div class="studio-quick-actions">
            <!-- Order is explicit product direction (setup-first: get providers
                 configured, then the cross-tab tools, then project/workflow
                 actions). Only the markup order changed — every id, icon,
                 label, and its wiring below is untouched. -->
            <button class="ghost-btn small" id="studioKeysProvidersBtn">${i(`key`,13)} Keys &amp; Providers</button>
            <button class="ghost-btn small" id="studioSymbiosisBtn">${i(`symbiosis`,13)} Symbiosis</button>
            <button class="ghost-btn small" id="studioModelsHubBtn">${i(`models`,13)} Models Hub</button>
            <button class="ghost-btn small" id="studioAIPerformanceBtn">${i(`performance`,13)} AI Performance</button>
        ...
        <!-- Same shared two-section hint (AI Safety Notice + live Provider
             priority line) every generation tab renders — see
             Mode.renderPipelineHint. Studio previously omitted it, which
             made it the only left panel without the safety notice. 'text'
             is the correct type here because Studio's own AI surfaces (Ask
             MAGENAIS, the Intelligence workspaces) route through the text
             provider pool, exactly like the Coding/Game tabs do. -->
