...
`)}async function A(e,t,r){let i=t.participants.filter(e=>e.enabled);if(i.length===0)return{ok:!1,error:`No enabled participants to generate a summary with.`};let a=e.getProviderRegistry(),o=e.getProviderManager(),s=k(t),c=``;for(let e of i){let t=o.getProviders(`text`,!1).find(t=>t.id===e.providerId);if(!t||!t.enabled||!(t.noKeyNeeded||t.apiKey)){c=`Provider "${e.providerName}" is unavailable.`;continue}let i=a.getAdapter(t.adapterId);if(!i?.call){c=`Provider "${e.providerName}" has no callable adapter.`;continue}try{let a=6e3,o=e.model?{model:e.model,maxTokens:a,log:r||(()=>{})}:{maxTokens:a,log:r||(()=>{})},l=n.newRequestId(),u=Date.now(),d=await E(i,t,s,o);if(d&&d.trim())return n.event({requestId:l,provider:e.providerName,model:e.model,providerType:`text`,winner:!0,latencyMs:Date.now()-u}),{ok:!0,content:d.trim()};n.event({requestId:l,provider:e.providerName,model:e.model,providerType:`text`,failureReason:`Empty response`}),c=`Empty response from ${e.providerName}.`}catch(e){c=e?.message||String(e)}}return{ok:!1,error:c||`Unable to generate a summary.`}}function j(e){let t=new Map,n=[];for(let r of e.transcript){let e=r.kind===`human`?`human:${r.id}`:`ai:${r.round}:${r.participantId}`;t.has(e)||n.push(e),t.set(e,r)}return n.map(e=>t.get(e))}function M(e){return new Date(e).toLocaleString()}function N(e){let t=`**${e.kind===`human`?`Human`:`${e.roleLabel} (${e.providerName}${e.model?` / `+e.model:``})`}** — round ${e.round}, ${M(e.timestamp)}`;if(e.status===`skipped`)return`${t}\n\n_Skipped._`;if(e.status===`error`)return`${t}\n\n_No response — ${e.error||`error`}._`;let n=e.assetRef?`\n\n_Reference: ${e.assetRef.label} (${e.sourceTab||e.assetRef.assetType})_`:``;return`${t}\n\n${e.content}${n}`}function P(e){let t=[];t.push(`# ${e.title}`),t.push(``),t.push(`**Topic:** ${e.topic||`(none stated)`}`),t.push(`**Date:** ${M(e.createdAt)} (last updated ${M(e.updatedAt)})`),t.push(`**Mode:** ${e.mode===`multi-provider`?`Multi-Provider Council`:`Single-Provider Multi-Role Council`}`),t.push(`**Status:** ${e.status}`),t.push(``),t.push(`## Participants`),t.push(``);for(let n of e.participants)t.push(`- **${n.roleLabel}** — ${n.providerName}${n.model?` / `+n.model:``}${n.enabled?``:` (disabled)`}`);e.mode===`single-provider`&&(t.push(``),t.push(`_Only one provider/model is available for this project. These are different reasoning roles from the same provider/model, not independent AI systems._`)),t.push(``),e.summary&&(t.push(`## Summary`),t.push(``),t.push(e.summary),t.push(``)),t.push(`## Full Transcript`),t.push(``);let n=j(e),r=Array.from(new Set(n.map(e=>e.round))).sort((e,t)=>e-t);for(let e of r){t.push(`### Round ${e}`),t.push(``);for(let r of n.filter(t=>t.round===e))t.push(N(r)),t.push(``)}return t.join(`
`)}function F(e){return e.replace(/[^a-z0-9\-_ ]/gi,``).trim().replace(/\s+/g,`-`).slice(0,60)||`symbiosis-project`}async function I(e,t){let n=F(e.title),a=P(e);switch(t){case`docx`:return{blob:await r(e.title,a),filename:`${n}.docx`};case`pdf`:return{blob:await i(e.title,a),filename:`${n}.pdf`};case`md`:return{blob:new Blob([a],{type:`text/markdown`}),filename:`${n}.md`};case`txt`:{let e=a.replace(/^#{1,6}\s*/gm,``).replace(/\*\*/g,``);return{blob:new Blob([e],{type:`text/plain`}),filename:`${n}.txt`}}case`json`:return{blob:new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),filename:`${n}.json`}}}var L=class t extends a{view=`list`;currentProjectId=null;isRunning=!1;stopRequested=!1;pendingHandoffBanner=null;pendingAttachmentRef=null;get store(){return this.kernel.getSymbiosisStore()}activate(){let e=o();e&&(this.pendingHandoffBanner=e);let t=this.store.getActiveProject();t&&t.status!==`archived`?(this.currentProjectId=t.id,this.view=t.participants.length===0?`configure`:`discussion`):this.view=`list`,this.render()}deactivate(){}getTitle(){return`Symbiosis`}onReselect(){this.render()}render(){switch(this.renderControl(this.controlMarkup()),this.wireControl(),this.view){case`list`:this.renderListView();break;case`create`:this.renderCreateView();break;case`configure`:this.renderConfigureView();break;case`discussion`:this.renderDiscussionView()}}controlMarkup(){let e=this.currentProjectId?this.store.getProject(this.currentProjectId):void 0,t=this.store.getProjects().filter(t=>t.id!==e?.id);return`
      <div class="field">
    ...
    `}jumpToMarkup(e){let t=Array.from(new Set(e.transcript.map(e=>e.round))).sort((e,t)=>e-t);return t.length===0?``:`
      <div class="field">
        <label class="field-label" for="symJumpTo">Jump to</label>
        <select id="symJumpTo">
          <option value="">— round or role —</option>
          <optgroup label="Rounds">${t.map(e=>`<option value="round-${e}">Round ${e}</option>`).join(``)}</optgroup>
          <optgroup label="Roles (latest turn)">${e.participants.map(e=>`<option value="role-${e.id}">${this.escapeHtml(e.roleLabel)} (latest turn)</option>`).join(``)}</optgroup>
        </select>
      </div>
    ...
    `}wireControl(){this.controlPanel.querySelector(`#symBackToList`)?.addEventListener(`click`,()=>{this.view=`list`,this.currentProjectId=null,this.render()}),this.controlPanel.querySelector(`#symNewProject`)?.addEventListener(`click`,()=>{this.view=`create`,this.render()}),this.controlPanel.querySelector(`#symOpenProjects`)?.addEventListener(`click`,()=>{this.view=`list`,this.render()}),this.controlPanel.querySelector(`#symProjectSwitcher`)?.addEventListener(`change`,e=>{let t=e.target.value;t&&this.openProject(t)}),this.controlPanel.querySelector(`#symJumpTo`)?.addEventListener(`change`,e=>{let t=e.target.value;t&&(document.getElementById(`sym${t.startsWith(`round-`)?`Round`:`Role`}-${t.split(`-`).slice(1).join(`-`)}`)?.scrollIntoView({behavior:`smooth`,block:`start`}),e.target.value=``)})}renderListView(){let e=this.store.getProjects(),t=this.pendingHandoffBanner?this.pendingHandoffBannerMarkup(this.pendingHandoffBanner):``,n=e.length===0?`<p class="hint" style="padding:12px 0;">No Symbiosis projects yet. Create one to bring AI perspectives together on a topic.</p>`:e.map(e=>this.projectRowMarkup(e)).join(``);this.renderOutput(`
...
`),r=prompt(`Add to which project?\n${n}`,`1`),i=t[r?parseInt(r,10)-1:-1];i&&(this.store.appendTranscriptEntry(i.id,{round:i.currentRound||1,kind:`human`,content:e.content,timestamp:Date.now(),status:`ok`,sourceTab:e.sourceTab,assetRef:e.assetRef}),this.store.setActiveProject(i.id),this.pendingHandoffBanner=null,this.openProject(i.id))}))}renderCreateView(){this.renderOutput(`
    ...
    `),this.wireConfigureView(e,n)}participantCardMarkup(e,t,n){let r=l.map(t=>`<option value="${t.id}" ${t.id===e.roleId?`selected`:``}>${t.label}</option>`).join(``),i=n.map(t=>`<option value="${t.id}" ${t.id===e.providerId?`selected`:``}>${this.escapeHtml(t.name)}${t.model?` — `+this.escapeHtml(t.model):``}</option>`).join(``);return`
      <div class="save-bar symb-row" style="display:flex; gap:10px; justify-content:space-between; align-items:flex-start; flex-wrap:wrap;" data-participant-card="${e.id}">
    ...
    `),this.wireDiscussionView(e)}seatsStripMarkup(e,t){return`<div class="symb-seats">${e.participants.map((n,r)=>{let i=this.seatColor(r),a=[...e.transcript].reverse().find(e=>e.round===t&&e.participantId===n.id),o=`○`,s=`Pending`,c=`var(--ink-faint)`;return n.enabled?t===0?(o=`·`,s=`Seated`):a?.status===`ok`?(o=`✓`,s=`Responded`,c=`var(--moss)`):a?.status===`error`?(o=`✕`,s=`No response`,c=`var(--rust)`):a?.status===`skipped`?(o=`—`,s=`Skipped`):this.isRunning&&(o=`…`,s=`Thinking`,c=`var(--amber)`):(o=`—`,s=`Disabled`),`
        ...
        </div>`}).join(``)}<div class="symb-seat symb-seat-human"><div class="symb-seat-role">You</div><div class="hint" style="font-size:11px;">Human</div></div></div>`}transcriptMarkup(e){if(e.transcript.length===0)return`<p class="hint">No turns yet. Press Start to begin round 1.</p>`;let t=j(e),n=Array.from(new Set(t.map(e=>e.round))).sort((e,t)=>e-t),r=new Map;for(let e of t)e.participantId&&r.set(e.participantId,e.id);return n.map(n=>`
    ...
    `).join(``)}entryMarkup(e,t,n=!1){let r=e.kind===`human`,i=!r&&n&&e.participantId?` id="symRole-${e.participantId}"`:``,a=e.participantId?t.participants.findIndex(t=>t.id===e.participantId):-1,o=r?`var(--ink-dim)`:this.seatColor(a>=0?a:0),s=r?`You`:this.escapeHtml(e.roleLabel||`Participant`),c=r?e.sourceTab?`via ${this.escapeHtml(e.sourceTab)}`:``:`${this.escapeHtml(e.providerName||``)}${e.model?` / `+this.escapeHtml(e.model):``}`,l,u=``;return e.status===`error`?(l=`<span style="color:var(--rust);">No response — ${this.escapeHtml(e.error||`error`)}</span>`,e.participantId&&(u=`
      ...
      <div class="symb-bubble-row ${r?`symb-bubble-row-human`:``}"${i}>
