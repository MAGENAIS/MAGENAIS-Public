import{d as e}from"./main-17o5_Sre.js";import{t}from"./Mode-2BXbs3UA.js";import{i as n,r}from"./SymbiosisHandoff-DkpRyW56.js";var i=[{key:`semanticscholar`,label:`Semantic Scholar`},{key:`openalex`,label:`OpenAlex`},{key:`arxiv`,label:`arXiv`}],a=class extends t{activate(){this.renderControl(`
      <div class="field">
      ...
      <div class="field">
        ...
        <div class="chip-group" id="researchSourceChips">
          ${i.map(e=>`<span class="chip active" data-val="${e.key}">${e.label}</span>`).join(``)}
        </div>
      </div>
      <div class="field">
    ...
    `),document.querySelectorAll(`#researchSourceChips .chip`).forEach(e=>{e.addEventListener(`click`,()=>{let t=document.querySelectorAll(`#researchSourceChips .chip.active`).length;e.classList.contains(`active`)&&t<=1||e.classList.toggle(`active`)})});let e=document.getElementById(`researchCount`);e?.addEventListener(`input`,()=>{let t=document.getElementById(`researchCountVal`);t&&(t.textContent=e.value)}),document.getElementById(`runBtn`)?.addEventListener(`click`,()=>this.runGuarded(`runBtn`,()=>this.handleRun()))}async handleRun(){let e=document.getElementById(`promptInput`)?.value.trim();if(!e){alert(`Enter a research question.`);return}let t=Array.from(document.querySelectorAll(`#researchSourceChips .chip.active`)).map(e=>e.dataset.val).filter(Boolean),n=parseInt(document.getElementById(`researchCount`)?.value||`6`,10);this.outputPanel.querySelector(`.stage`)&&this.renderLoading(`Gathering sources…`);let r={id:`research-`+Date.now(),name:`Research`,graph:{nodes:[{id:`research1`,type:`research`,label:`Research Agent`,config:{sources:t,limitPerSource:n},inputs:{query:e},enabled:!0}],edges:[]},createdAt:Date.now(),updatedAt:Date.now()};try{let t=await this.kernel.getWorkflowEngine().execute(r,{query:e},(e,t)=>this.appendLog(e,t));this.renderResult(t.finalOutput,e),this.kernel.getStore().getActions().addHistoryEntry({mode:`research`,prompt:e,result:t.finalOutput?.summary,resultType:`text`})}catch(e){this.renderError(e)}}renderResult(t,i){let a=this.outputPanel.querySelector(`.stage`);if(!a)return;let{papers:o,sourceStatus:s,summary:c}=t||{},l=``;s&&(l+=`<div class="chip-group" style="margin-bottom:12px;">${s.map(e=>`<span class="chip ${e.status===`ok`?`active`:``}" style="${e.status===`fail`?`opacity:.5;`:``}">${e.name} ${e.status===`ok`?`✓`:`✕`}</span>`).join(``)}</div>`),c&&(l+=`<div class="doc-summary-block" style="margin-bottom:18px;"><div class="result-text">${this.renderMarkdown(c)}</div>${this.renderReadAloudBlock(e(c),`Read Summary Aloud`)}</div>`),o?.length&&(l+=`<p class="field-label">Sources (${o.length})</p>`,l+=o.slice(0,20).map(e=>`
      ...
      `).join(``));let u=`research-${Date.now()}`,d=!!(c||o?.length);d&&(l+=this.renderSaveBar(u)),d&&(l+=`<div class="studio-quick-actions" style="margin-top:8px;">${r(`Send Result to Symbiosis`)}</div>`),a.innerHTML=l||`<div class="empty-text">No results.</div>`,this.wireReadAloudControls(),this.wireCodeCopyButtons(a),d&&(this.wireSaveBar(a,u,`research`,()=>i||`Research result`,()=>({summary:c,papers:o,sourceStatus:s})),n(this.kernel,a,()=>({sourceTab:`Research`,title:i||`Research result`,content:c||o?.map(e=>`${e.title} (${e.source})`).join(`
`)||``})))}deactivate(){typeof window<`u`&&window.speechSynthesis&&window.speechSynthesis.cancel()}getTitle(){return`Research`}};export{a as ResearchMode};