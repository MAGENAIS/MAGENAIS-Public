function g(e,t){return`Here is the complete current HTML/JS source of a small browser game:
...
Output ONLY the full, complete, updated HTML document (starting with <!DOCTYPE html>, ending with </html>) with the requested change applied — no explanation, no markdown code fences. Keep everything else about the game working exactly as before unless the change naturally requires touching it. Make sure the result is still a single self-contained file.`}function u(e){let t=e.trim();const n=t.match(/```(?:html)?\s*([\s\S]*?)```/);n&&(t=n[1].trim());const a=t.search(/<!DOCTYPE html>/i);a>0&&(t=t.slice(a));const o=/<!DOCTYPE html>/i.test(t),s=/<\/html>/i.test(t);if(!o||!s){const r=e.trim().slice(0,140).replace(/\s+/g," "),i=o?"found a start but the document never closed with </html> (likely cut off)":"no <!DOCTYPE html> found anywhere in the response";throw new Error(`Not a complete HTML document (${i}). What came back started with: "${r}${e.length>140?"…":""}"`)}return t}function p(e){let t=e.trim();const n=t.match(/```(?:js|javascript|html)?\s*([\s\S]*?)```/);return n&&(t=n[1].trim()),t}function y(e){if(!/THREE\.(Scene|WebGLRenderer|PerspectiveCamera)/.test(e))throw new Error("3D engine was requested but the generated code has no Three.js scene/renderer/camera setup.");if(!/THREE\.(AmbientLight|DirectionalLight|PointLight|SpotLight|HemisphereLight)/.test(e))throw new Error("3D scene has no lighting (THREE.*Light) — meshes would render flat/unlit regardless of geometry.");const a=/MeshStandardMaterial|MeshPhongMaterial|MeshLambertMaterial|MeshPhysicalMaterial/.test(e);if(/MeshBasicMaterial/.test(e)&&!a)throw new Error("3D scene only uses MeshBasicMaterial, which ignores lighting and renders flat/2D-looking.");if(!/THREE\.PlaneGeometry/.test(e))throw new Error("3D scene has no ground/floor plane (THREE.PlaneGeometry) — entities would appear to float in empty space with nothing to stand on.");if(!/camera\.lookAt\s*\(/.test(e))throw new Error("3D scene never calls camera.lookAt(...) — the camera would keep its default orientation instead of framing the ground/action, which is what makes scenes look like they're pointed at empty sky.");if([...e.matchAll(/camera\.position\.(?:set\s*\(\s*[^,]+,\s*(-?\d+(?:\.\d+)?)|y\s*=\s*(-?\d+(?:\.\d+)?))/g)].some(l=>{const h=parseFloat(l[1]??l[2]);return!isNaN(h)&&h>40}))throw new Error("Camera is positioned unrealistically high above the scene (Y > 40), which fills most of the frame with empty sky instead of the ground/action.");return e}async function d(e,t,n,a,o,s=3){let r;for(let i=1;i<=s;i++)try{i>1&&a(`${o} — retry ${i}/${s}…`,"warn");const c=i>1?`

(attempt id: ${Math.random().toString(36).slice(2)} — ignore this line, it's only here to ensure a fresh response)`:"",l=await n(e()+c,{temperature:.7});return t(l)}catch(c){r=c,a(`${o} attempt ${i} failed: ${c.message}`,"error")}throw r}function f(e){return e.engine==="3d"?`
...
loop();`}function w(e){if(!/getContext\s*\(\s*['"]2d['"]\s*\)/.test(e))throw new Error('2D engine was requested but the generated code never calls canvas.getContext("2d").');if(!/requestAnimationFrame/.test(e))throw new Error("2D game has no requestAnimationFrame game loop — it would only ever draw a single static frame.");if(!/fillText\s*\(/.test(e))throw new Error("2D game never calls ctx.fillText — entities would not be drawn as the required emoji/Unicode sprites (likely rendering as plain shapes or nothing).");return e}function E(e){return`Write ONLY the HTML structure, CSS styling, and UI scaffolding (no game logic JS yet) for a simple browser game. ${e.engine==="3d"?'This will be a 3D game using Three.js. Include exactly this script tag in the <head>: <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"><\/script>':'This will be a 2D game using the HTML5 Canvas API — include a <canvas id="gameCanvas"></canvas> element.'}
...
- CAMERA AIM: set camera.position and call camera.lookAt(...) once during setup, and again every frame in the animation loop, targeting the player/action's current position (not the origin if the player moves). Keep the camera LOW and CLOSE: position it roughly 5-10 units above the ground and 8-15 units back from the player, never higher — a camera positioned much higher than this, or angled steeply downward at the horizon instead of at the player, is what causes the frame to fill mostly with empty sky/background instead of the ground and entities, which is a common failure to avoid here.`:`Every distinct entity type from the concept (player, obstacles, collectibles, enemies, etc.) MUST be drawn as a large, readable emoji or Unicode symbol via ctx.font (e.g. "32px serif") + ctx.fillText at its position — pick the emoji that actually matches what the concept describes (a frog game's player should visibly be a frog emoji like 🐸, its obstacles should visibly be stones like 🪨, etc.), not a plain filled rectangle/circle. This is the single most important requirement: the concept must be immediately recognizable just by looking at the game, not just functionally implemented.`,a=e.engine==="3d"?"Three.js (r128 — no OrbitControls, no CapsuleGeometry; use BoxGeometry/SphereGeometry/CylinderGeometry and manual camera control)":"the HTML5 Canvas 2D API",o=e.genre?` Genre direction: ${e.genre}.`:"",s=e.complexity==="minimal"?" Keep it minimal: just the core playable loop, one win/lose condition.":e.complexity==="rich"?" Add modest depth: simple scoring/progression and a couple of varied obstacle/enemy types, but stay realistic for one pass.":" Standard scope: a clear core loop and one simple scoring or win condition.";return`Here is the HTML/CSS scaffolding already built for a browser game (DO NOT repeat or rewrite this — you're only writing the JS that goes where the placeholder comment is):

\`\`\`html
${t}
\`\`\`

Game concept: "${e.concept}" using ${a}.${o}${s}
...
Output ONLY the raw JavaScript code — no <script> tags, no explanation, no markdown fences, just the code that goes inside the script tag.`}async function v(e,t,n){if(e.iterate&&e.previousCode){t("Applying your change request to the existing game…");const r=g(e.previousCode,e.concept),i=await d(()=>r,u,n,t,"Iteration");return m(i)}t("Stage 1/2 — structure agent: building HTML/CSS scaffolding…");const a=await d(()=>E(e),u,n,t,"Structure agent");a.includes("GAME_LOGIC_PLACEHOLDER")||t("Structure agent didn't include the expected placeholder — proceeding anyway, logic agent will target the last <script> tag.","warn"),t(`Stage 2/2 — logic agent: writing the ${e.engine==="3d"?"3D":"2D"} game loop…`);let o;try{o=await d(()=>b(e,a),e.engine==="3d"?r=>y(p(r)):r=>w(p(r)),n,t,"Logic agent")}catch(r){t(`Logic agent could not produce a validated ${e.engine==="3d"?"3D":"2D"} game after retries (${r?.message||r}) — using a guaranteed-working fallback game loop instead so you still get something playable.`,"warn"),o=f(e)}const s=a.includes("GAME_LOGIC_PLACEHOLDER")?a.replace("// GAME_LOGIC_PLACEHOLDER",()=>o):a.replace(/<script>\s*<\/script>/i,()=>`<script>${o}<\/script>`);return t("Assembled final game from both stages.","info"),m(s)}function m(e){const t=`<script>
...
<\/script>`;return/<\/body>/i.test(e)?e.replace(/<\/body>/i,()=>t+"</body>"):e+t}export{v as generateGame};
