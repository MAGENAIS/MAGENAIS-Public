const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/podcast-DH9cI4_D.js","assets/main-D49q3yve.js","assets/rolldown-runtime-Dd_uD5pT.js","assets/Environment-DAaQRBRU.js","assets/main-DM-5SoV6.css"])))=>i.map(i=>d[i]);
import{O as e,d as t}from"./main-D49q3yve.js";import{t as n}from"./Mode-GEMYOo3Y.js";import{i as r,r as i}from"./SymbiosisHandoff-DkpRyW56.js";import{t as a}from"./VoiceInput-CvaLwtaL.js";var o=class extends n{activeMode=`talk`;conversation=[];activate(){this.renderControl(`
      <div class="field">
        <label class="field-label">Mode</label>
        <div class="chip-group" id="audioModeChips">
          <span class="chip active" data-val="talk">Talk</span>
          <span class="chip" data-val="speech">Speech</span>
      ...
      -->
      <div class="field" id="audioTalkSection">
        ...
        <div class="field" style="margin-top:10px;">
          <label class="field-label">Or type your turn</label>
          <div style="display:flex; gap:8px; align-items:flex-end;">
            <textarea id="audioTalkTextInput" rows="2" placeholder="Type a message instead of speaking…" style="flex:1;"></textarea>
            <button class="ghost-btn" id="audioTalkSendBtn" title="Send typed message">Send</button>
          </div>
          <p class="hint">Typed and spoken turns share the same conversation — the reply is still read aloud, exactly like a spoken turn.</p>
        </div>
        <div id="audioTalkPipelineHint">${this.renderPipelineHint(`text`)}</div>
      </div>

      <div class="field" id="audioPromptField" style="display:none;">
        <label class="field-label">Text to speak</label>
        <textarea id="promptInput" rows="5" placeholder="Welcome to MAGENAIS..."></textarea>
      </div>
      <div class="field" id="audioVoiceField" style="display:none;">
...
Host B: Thanks for having me...`:`Paste your full narration script here.`}wireTalk(){let e=document.getElementById(`audioTalkBtn`),t=document.getElementById(`audioTalkStatus`);e&&a(this.kernel,e,t,e=>{this.handleTalkTranscript(e)}),document.getElementById(`audioTalkClearBtn`)?.addEventListener(`click`,()=>{this.conversation=[],window.speechSynthesis?.cancel();let e=this.outputPanel.querySelector(`.stage`);e&&(e.innerHTML=`<div class="empty-glyph">◇</div><div class="empty-text">Conversation cleared. Click 🎙 Talk to start a new one.</div>`),this.setTalkStatus(`Click, then speak — your words and the spoken reply appear in the output panel as the conversation happens.`)}),document.getElementById(`audioTalkPlayBtn`)?.addEventListener(`click`,()=>this.talkPlay()),document.getElementById(`audioTalkPauseBtn`)?.addEventListener(`click`,()=>this.talkPause()),document.getElementById(`audioTalkStopBtn`)?.addEventListener(`click`,()=>this.talkStop());let n=document.getElementById(`audioTalkTextInput`),r=()=>{let e=n?.value.trim();e&&(n.value=``,this.handleTalkTranscript(e))};document.getElementById(`audioTalkSendBtn`)?.addEventListener(`click`,r),n?.addEventListener(`keydown`,e=>{e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),r())})}resolveTalkAudioEl(){let e=document.getElementById(`audioTalkTranscript`);if(!e)return null;let t=Array.from(e.querySelectorAll(`audio`));return t.length===0?null:t.find(e=>!e.paused&&!e.ended)||t[t.length-1]}talkPlay(){let e=window.speechSynthesis;if(e?.paused){e.resume(),this.setTalkStatus(`Speaking reply…`);return}let t=this.resolveTalkAudioEl();if(t&&!t.ended){t.play(),this.setTalkStatus(`Speaking reply…`);return}this.setTalkStatus(`Nothing to resume yet — speak a question first.`)}talkPause(){let e=window.speechSynthesis;if(e?.speaking&&!e.paused){e.pause(),this.setTalkStatus(`Paused. Click ▶ Play to continue.`);return}let t=this.resolveTalkAudioEl();if(t&&!t.paused){t.pause(),this.setTalkStatus(`Paused. Click ▶ Play to continue.`);return}this.setTalkStatus(`Nothing is currently playing.`)}talkStop(){window.speechSynthesis?.cancel(),document.getElementById(`audioTalkTranscript`)?.querySelectorAll(`audio`).forEach(e=>{let t=e;t.pause(),t.currentTime=0}),this.setTalkStatus(`Playback stopped. Click the mic to speak again, or ▶ Play to replay the last reply.`)}setTalkStatus(e){let t=document.getElementById(`audioTalkStatus`);t&&(t.textContent=e)}ensureTalkTranscript(){let e=document.getElementById(`audioTalkTranscript`);if(e)return e;let t=this.outputPanel.querySelector(`.stage`);return t.innerHTML=`
      <div class="doc-summary-block">
        ...
        ${this.renderSaveBar(`audio-talk-conversation`)}
        <div class="studio-quick-actions" style="margin-top:8px;">${i(`Send Audio Result`)}</div>
      </div>`,this.outputPanel.scrollTop=0,this.wireSaveBar(t,`audio-talk-conversation`,`text`,()=>`Talk conversation — ${new Date().toLocaleString()}`,()=>this.conversation.map(e=>`### ${e.role===`user`?`You`:`Assistant`}\n\n${e.text}`).join(`

`)),r(this.kernel,t,()=>({sourceTab:`Audio`,title:`Talk conversation — ${new Date().toLocaleString()}`,content:this.conversation.map(e=>`${e.role===`user`?`You`:`Assistant`}: ${e.text}`).join(`

`)})),document.getElementById(`audioTalkTranscript`)}appendTalkTurn(e,t){let n=this.ensureTalkTranscript(),r=document.createElement(`div`),i=e===`user`;return r.style.cssText=`align-self:${i?`flex-end`:`flex-start`}; max-width:85%; background:${i?`var(--bg-raised)`:`transparent`}; border:1px solid var(--line-bright); border-radius:var(--radius); padding:8px 12px;`,r.innerHTML=`
