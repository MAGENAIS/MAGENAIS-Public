import{b as e,y as t}from"./main-DOB2GXWU.js";import{t as n}from"./Mode-BozB-yO3.js";import{i as r,r as i}from"./SymbiosisHandoff-DkpRyW56.js";var a=[`H`,`X`,`Y`,`Z`,`S`,`T`],o=[`RX`,`RY`,`RZ`],s=class extends n{numQubits=2;ops=[];shots=1024;activeTab=`build`;activate(){this.renderControl(this.controlMarkup()),this.wireTabs(),this.wireBuildControls(),this.wireAiControls(),this.renderCircuitStage()}deactivate(){}getTitle(){return`Quantum`}controlMarkup(){return`
      ...
      <div id="quantumBuildPanel" class="field">
        ...
        <div class="chip-group" id="qSingleGates">
          ${a.map(e=>`<button class="chip small" data-gate="${e}">${e}</button>`).join(``)}
        </div>
        <div class="studio-quick-actions" style="margin-top:6px;">
          ${o.map(e=>`<button class="ghost-btn small" data-rot-gate="${e}">${e}(&theta;)</button>`).join(``)}
        ...
        <details class="adv" style="margin-top:10px;">
          ...
          <div class="adv-body">
            <p class="hint">MAGENAIS runs circuits on exactly two backends: the free local simulator above, and real IBM Quantum hardware once you configure that provider. Everything below is a real, commercially available quantum service that MAGENAIS deliberately does <b>not</b> claim to connect to — a static, backend-less browser app cannot reach them with nothing but a pasted API key, for the specific reason given under each one. Rather than ship presets that would fail with an opaque network error, they are listed here (and, with the same facts, under the <b>Quantum</b> filter in Keys &amp; Providers) as reference entries with official links. Free tiers change often — check each vendor's own page before relying on what is stated here.</p>
            ${this.backendCatalogMarkup()}
            <p class="hint" style="margin-top:10px;">Two practical notes. First, several of these are <i>not gate-model</i> machines — D-Wave anneals, QuEra and Pasqal run analog neutral-atom sequences, Xanadu and Quandela are photonic — so even with a working connection the Circuit Builder above would have nothing meaningful to submit to them. Second, the cheapest route onto real hardware is usually an aggregator (AWS Braket, Azure Quantum, qBraid) rather than a direct vendor account, because they resell several vendors under one billing relationship.</p>
          </div>
        </details>
      </div>
    ...
    `}backendCatalogMarkup(){return[{heading:`Free / free tier — usable today without payment details`,entries:t.filter(e=>e.tier===`free`||e.tier===`free-tier`)},{heading:`Free trial — time-boxed access`,entries:t.filter(e=>e.tier===`free-trial`)},{heading:`Commercial / intermediated access`,entries:t.filter(e=>e.tier===`paid`)}].filter(e=>e.entries.length>0).map(e=>`
      ...
      <div class="studio-mini-item" style="display:block; cursor:default; text-align:left; margin-bottom:6px;">
        ...
        <span class="hint" style="display:block;">${n.join(` &middot; `)}</span>
          ...
          </div>`,this.wireSaveBar(r,i,`quantum`,()=>`IBM Quantum: ${n.backend} (${n.jobId})`,()=>({circuit:this.getCircuit(),backend:n.backend,jobId:n.jobId,raw:n.raw}))),this.kernel.getStore().getActions().addHistoryEntry({mode:`quantum`,prompt:`[Hardware: ${n.backend}] ${this.numQubits}q, ${this.ops.length} gates`,result:JSON.stringify(n.raw),resultType:`text`})}catch(e){this.renderError(e)}}findIbmQuantumProvider(){return this.kernel.getProviderManager().getProviders(`quantum`,!0).find(e=>e.adapterId===`ibm-quantum`&&e.enabled)}updateHardwareButtonVisibility(){let e=this.controlPanel.querySelector(`#qHardwareBtn`);e&&(e.style.display=this.findIbmQuantumProvider()?`block`:`none`)}renderSimulationResult(e,t){let n=this.outputPanel.querySelector(`.stage`);if(!n)return;let a=Math.max(...Object.values(e.counts)),o=Object.entries(e.counts).sort((e,t)=>t[1]-e[1]).map(([t,n])=>{let r=(n/e.shots*100).toFixed(1),i=n/a*100;return`
          ...
          </div>`}).join(``),s=`quantum-${Date.now()}`;n.innerHTML=`
      <div class="result-text">
